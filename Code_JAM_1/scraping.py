...
c.execute('CREATE TABLE IF NOT EXISTS stuffToPlot(roll TEXT, username TEXT, name TEXT, program TEXT, dept TEXT, hall TEXT, room TEXT,blood_group TEXT, gender TEXT, hometown TEXT)')
s = requests.Session()
...
def process_response_soup(soup, c):
    for link in soup.select('.TableText a'):
        ...
        c.execute('REPLACE INTO stuffToPlot VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',(roll, username, name, program, dept, hall, room,blood_group, gender, hometown))
